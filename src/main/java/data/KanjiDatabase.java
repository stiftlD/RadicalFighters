...
public class KanjiDatabase {
    ...
    int currKanjiID = 0;
    private int generateKanjiID() { return currKanjiID++;}
    ...
    private class JsonKanji {
        ...
        public int getJLPT() { return this.jlpt;}

        public String getUnicode() {return this.unicode;}

        public String getHeisig() {return this.heisig_en;}
    }
    ...
    /*private class JsonKanji {
        ...
        public JsonKanji(int id, String character, List<Integer> grades, int strokes, String kunyomi, String translations, String radicalName) {
            this.id = id;
            this.character = character;
            this.grades = grades;
    ...
    public void initialize() throws SQLException {
        ...
        try
        {
            ...
            System.out.println(kanjiArray.length);
    ...
    public List<Kanji> getRandomKanjiInProfInterval(int maxGrade, double start, double end) throws SQLException, IllegalArgumentException {
        if (maxGrade < 0 || start < 0.0 || end > 1.0 || start > end) throw new IllegalArgumentException("start and end within [0, 1]");
        ...
        try (Connection connection = DriverManager.getConnection(kanjiDBURL)) {
            try (Statement statement = connection.createStatement()) {
                // create viable Kanji view TODO do this in init()
                try {
                    ResultSet viableView = statement.executeQuery(createViableKanjiView);
                } catch (SQLiteException e) {
                    e.printStackTrace();
